...
- [x] 2 Analisis baru — single form, input-completeness panel, fixed §15 wording — verified: `src/screens/Analyze.test.tsx`, 11 tests incl. QC batch upload and analysis linkage, the verbatim adoption sentence, the run button never disabled for missing input, and that it writes only the machine condition, reading the factory-wide context for coverage
